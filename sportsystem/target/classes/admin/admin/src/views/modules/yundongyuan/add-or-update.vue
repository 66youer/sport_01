	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="投票数" prop="toupiaoshu" >
					<el-input v-model.number="ruleForm.toupiaoshu" placeholder="投票数" clearable  :readonly="ro.toupiaoshu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="投票数" prop="toupiaoshu" >
					<el-input v-model="ruleForm.toupiaoshu" placeholder="投票数" readonly></el-input>
				</el-form-item>
			</template>
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					this.ruleForm.toupiaoshu = Number('0'); 				}
				// 获取用户信息
				this.$http({
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yundongyuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yundongyuanCrossAddOrUpdateFlag = false;
											this.parent.search();
