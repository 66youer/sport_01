	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xunlianjilu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">计划名称</div>
						<div class="text "  >{{detail.jihuamingcheng}}</div>
					</div>
					<div class="item">
						<div class="lable">类型</div>
						<div class="text "  >{{detail.leixing}}</div>
					</div>
					<div class="item">
						<div class="lable">训练记录</div>
						<div class="text "  >{{detail.xunlianjilu}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">教练工号</div>
						<div class="text "  >{{detail.jiaoliangonghao}}</div>
					</div>
					<div class="item">
						<div class="lable">上传时间</div>
						<div class="text "  >{{detail.shangchuanshijian}}</div>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xunlianjilu','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xunlianjilu','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('xunlianjilu','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/xunlianjilu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xunlianjiluAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此训练记录？') .then(_ => {
					this.$http.post('xunlianjilu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
