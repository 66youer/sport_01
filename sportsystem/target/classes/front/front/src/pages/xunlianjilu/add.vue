<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="计划名称" prop="jihuamingcheng">
				<el-input v-model="ruleForm.jihuamingcheng" 
					placeholder="计划名称" clearable :disabled=" false  ||ro.jihuamingcheng"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="类型" prop="leixing">
			...
			<el-form-item class="add-item" label="教练工号" prop="jiaoliangonghao">
				<el-input v-model="ruleForm.jiaoliangonghao" 
					placeholder="教练工号" clearable :disabled=" false  ||ro.jiaoliangonghao"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="上传时间" prop="shangchuanshijian">
				<el-date-picker
			...
			<el-form-item class="add-item" label="训练记录" prop="xunlianjilu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="训练记录"
					v-model="ruleForm.xunlianjilu">
					</el-input>
			</el-form-item>
...
<script>
	export default {
		data() {
			return {
				...
				rules: {
					...
					xunlianjilu: [
						{ required: true, message: '训练记录不能为空', trigger: 'blur' },
					],
					fengmian: [
					],
					shipin: [
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='xunlianjilu'){
							this.ruleForm.xunlianjilu = obj[o];
							this.ro.xunlianjilu = true;
							continue;
						}
						if(o=='fengmian'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`xunlianjilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xunlianjilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
