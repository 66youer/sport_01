	...
	<div>
		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('bisaixinxi','新增')" type="primary" @click="add('/index/bisaixinxiAdd')">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.xuanchuantupian)" v-if="item.xuanchuantupian && item.xuanchuantupian.substr(0,4)=='http'&& item.xuanchuantupian.split(',w').length>1" :src="item.xuanchuantupian" />
						<img class="image" @click.stop="imgPreView(item.xuanchuantupian.split(',')[0])" v-else-if="item.xuanchuantupian && item.xuanchuantupian.substr(0,4)=='http'" :src="item.xuanchuantupian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.xuanchuantupian?item.xuanchuantupian.split(',')[0]:''))" v-else :src="baseUrl + (item.xuanchuantupian?item.xuanchuantupian.split(',')[0]:'')" />
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('bisaileixing/list',{params: {sort: 'id',order: 'asc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'bisaileixing'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.curFenlei != '全部') searchWhere.bisaileixing = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`bisaixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/bisaixinxiDetail', query: params});
			},
