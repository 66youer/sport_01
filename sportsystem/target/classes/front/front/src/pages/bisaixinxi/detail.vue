	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/bisaixinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">比赛类型</div>
						<div class="text "  >{{detail.bisaileixing}}</div>
					</div>
					<div class="item">
						<div class="lable">比赛日期</div>
						<div class="text "  >{{detail.bisairiqi}}</div>
					</div>
					<div class="item">
						<div class="lable">比赛地点</div>
						<div class="text "  >{{detail.bisaididian}}</div>
					</div>
					<div class="item">
						<div class="lable">主办单位</div>
						<div class="text "  >{{detail.zhubandanwei}}</div>
					</div>
					<div class="item">
						<div class="lable">承办单位</div>
						<div class="text "  >{{detail.chengbandanwei}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('bisaixinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('bisaixinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('bisaixinxi','比赛报名')" 
							@click="onAcross('bisaibaoming','','','[1]','已报名')" type="warning">
							比赛报名
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="详情介绍" name="first">
					<div class="ql-snow ql-editor" v-html="detail.xiangqingjieshao"></div>
				</el-tab-pane>
						...
						<div class="comment-item" v-for="item in infoList" :key="item.id" @mouseenter="discussEnter(item.id)"
							...
							<div class="comment-content-box">
								...
								<div class="comment-btn">
									<!-- <el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px","color":"#fff","borderRadius":"0px","background":"#848c74","width":"auto","lineHeight":"32px","fontSize":"14px","minWidth":"90px","height":"32px"}'>回复</el-button> -->
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.bisaimingcheng;
						if(this.detail.xuanchuantupian) {
							this.detailBanner = this.detail.xuanchuantupian.split(",w").length>1?[this.detail.xuanchuantupian]:this.detail.xuanchuantupian.split(',');
						}
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('bisaixinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'bisaixinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'bisaixinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'bisaixinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/bisaixinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussbisaixinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussbisaixinxi/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussbisaixinxi/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussbisaixinxi/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/bisaixinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此比赛信息？') .then(_ => {
					this.$http.post('bisaixinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
