	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="投票数" prop="toupiaoshu">
				<el-input v-model.number="ruleForm.toupiaoshu" 
					placeholder="投票数" clearable :disabled="true  ||ro.toupiaoshu"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`yundongyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yundongyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
