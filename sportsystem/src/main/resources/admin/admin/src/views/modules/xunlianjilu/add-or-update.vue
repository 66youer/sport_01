	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="教练工号" prop="jiaoliangonghao" >
			...
			<el-form-item class="textarea" v-if="type!='info'" label="训练记录" prop="xunlianjilu" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.xunlianjilu" label="训练记录" prop="xunlianjilu" >
				<span class="text">{{ruleForm.xunlianjilu}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xunlianjilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xunlianjiluCrossAddOrUpdateFlag = false;
											this.parent.search();
