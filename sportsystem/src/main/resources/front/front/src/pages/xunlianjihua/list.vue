		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.jihuamingcheng" placeholder="计划名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('xunlianjihua','新增')" type="primary" @click="add('/index/xunlianjihuaAdd')">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="publisher_item">
							...
							<span class="text">{{item.jiaoliangonghao}}</span>
						</div>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.jihuamingcheng != '') searchWhere.jihuamingcheng = '%' + this.formSearch.jihuamingcheng + '%';
				if (this.formSearch.leixing != '') searchWhere.leixing = '%' + this.formSearch.leixing + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`xunlianjihua/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/xunlianjihuaDetail', query: params});
			},
