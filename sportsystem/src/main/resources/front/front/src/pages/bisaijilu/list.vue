	...
	<div>
		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('bisaijilu','新增')" type="primary" @click="add('/index/bisaijiluAdd')">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.bisaitupian)" v-if="item.bisaitupian && item.bisaitupian.substr(0,4)=='http'&& item.bisaitupian.split(',w').length>1" :src="item.bisaitupian" />
						<img class="image" @click.stop="imgPreView(item.bisaitupian.split(',')[0])" v-else-if="item.bisaitupian && item.bisaitupian.substr(0,4)=='http'" :src="item.bisaitupian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.bisaitupian?item.bisaitupian.split(',')[0]:''))" v-else :src="baseUrl + (item.bisaitupian?item.bisaitupian.split(',')[0]:'')" />
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`bisaijilu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/bisaijiluDetail', query: params});
			},
